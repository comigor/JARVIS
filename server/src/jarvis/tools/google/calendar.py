...
class ListEventsSchema(BaseModel):
    from_datetime: datetime = Field(description='From which timestamp you want the events (RFC3339 timestamp with mandatory time zone offset, e.g., 2011-06-03T10:00:00-07:00, 2011-06-03T10:00:00Z). Required.')
    to_datetime: datetime = Field(description='To which timestamp you want the events (RFC3339 timestamp with mandatory time zone offset, e.g., 2011-06-03T10:00:00-07:00, 2011-06-03T10:00:00Z). Required.')
...
class CreateEventSchema(BaseModel):
    ...
    start_datetime: datetime = Field(description='The start timestamp, as a combined date-time value (formatted according to RFC3339). Required.')
    end_datetime: datetime = Field(description='The end timestamp, as a combined date-time value (formatted according to RFC3339). Required.')
    location: Optional[str] = Field(description='Geographic location of the event as free-form text. Optional.')
...
class CreateEventTool(BaseTool):
    ...
    async def _arun(self, summary: str, start_datetime: datetime, end_datetime: datetime, location: str = None):
        creds = authenticate_with_google()
